...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.dec2 = nn.TransformerDecoderLayer(d_model=configs.d_model, dim_feedforward=configs.d_ff, nhead=configs.n_heads, batch_first=True)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if trainn:
            ...
            if mode == "Classification":
                ...
                linear_memory  = self.linearize(enc_out).detach()
                res = self.dec2(output.detach() if not backprop else output, linear_memory)
                res = self.linearize2(res)
                return torch.sigmoid(res)
            output = torch.zeros(32, 1, 129).cuda()
            linear_memory  = (enc_out)
            for i in range(24):
                #print(i)
                #print(linear_memory.size(), output.size())
                res = self.dec(self.dev_embedding(output.detach()) if not backprop else self.dec_embedding(output), linear_memory)
