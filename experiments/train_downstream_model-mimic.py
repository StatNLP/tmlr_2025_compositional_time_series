...
parser2.add_argument('--gold_path_train', type=str, default='/home/mitarb/hagmann/projects/structures_compositionality/gold_data/eicu-full/gold_train.pickle', help='root path of the data file')
#parser2.add_argument('--gold_path_val', type=str, default='very_important_blub.pkl', help='root path of the data file')
parser2.add_argument('--gold_path_val', type=str, default='/home/mitarb/hagmann/projects/structures_compositionality/gold_data/eicu-full/gold_valid.pickle', help='root path of the data file')
...
import os
if os.path.exists("eicu_indices.npy"):
    indices = np.load("eicu_indices.npy")
else:
    indices = np.array(list(range(len(fore_train_op))))
    np.random.shuffle(indices)
    np.save("eicu_indices.npy", indices)
...
var_to_ind = {'ALP': 1, 'ALT': 2, 'AST': 3, 'Albumin': 4, 'Anion Gap': 5, 'BUN': 6,
               'Base Deficit': 7, 'Base Excess': 8, 'Basophils': 9, 'Bedside Glucose': 10,
               'Bicarbonate': 11, 'Bilirubin (Direct)': 12, 'Bilirubin (Total)': 13,
               'Bodyweight (kg)': 14, 'CO2 (Total)': 15, 'Calcium': 16, 'Chloride': 17,
               'Creatinine (Blood)': 18, 'Creatinine (Urine)': 19, 'DBP': 20,
               'Eoisinophils': 21, 'EtCO2': 22, 'FiO2': 23, 'Fibrinogen': 24,
               'GCS eye': 25, 'GCS motor': 26, 'GCS verbal': 27, 'Glucose': 28,
               'HR': 29, 'Hct': 30, 'Hgb': 31, 'INR': 32, 'Lactate': 33,
               'Lymphocytes': 34, 'MBP': 35, 'MCH': 36, 'MCHC': 37, 'MCV': 38,
               'MPV': 39, 'Magnesium': 40, 'Monocytes': 41, 'Neutrophils': 42,
               'O2 L/%': 43, 'O2 Saturation': 44, 'PT': 45, 'PTT': 46, 'PaCO2': 47,
               'PaO2': 48, 'Phosphate': 49, 'Platelets': 50, 'Potassium': 51,
               'Protein (Total)': 52, 'RBC': 53, 'RDW': 54, 'RR': 55, 'SBP': 56,
               'Sodium': 57, 'Stool': 58, 'Temperature': 59, 'Troponin - I': 60,
               'Urine': 61, 'WBC': 62,'d Amiodarone': 63, 'd Dobutamine dose': 64,
               'd Dobutamine ratio': 65, 'd Dopamine dose': 66, 'd Dopamine ratio': 67,
               'd Epinephrine dose': 68, 'd Epinephrine ratio': 69, 'd Fentanyl 1': 70,
               'd Fentanyl 2': 71, 'd Fentanyl 3': 72, 'd Furosemide': 73, 'd Heparin 1': 74,
               'd Heparin 2': 75, 'd Heparin 3': 76, 'd Heparin vol': 77, 'd Insulin 1': 78,
               'd Insulin 2': 79, 'd Insulin 3': 80, 'd Midazolam 1': 81, 'd Midazolam 2': 82,
               'd Milrinone 1': 83, 'd Milrinone 2': 84, 'd Nitroglycerin 1': 85,
               'd Nitroglycerin 2': 86, 'd Nitroprusside': 87, 'd Norepinephrine 1': 88,
               'd Norepinephrine 2': 89, 'd Norepinephrine ratio': 90, 'd Pantoprazole': 91,
               'd Propofol 1': 92, 'd Propofol 2': 93, 'd Propofol 3': 94, 'd Vasopressin 1': 95,
               'd Vasopressin 2': 96, 'd Vasopressin 3': 97, 'pH': 98}
...
parser.add_argument('--enc_in', type=int, default=196, help='encoder input size') # DLinear with --individual, use this hyperparameter as the number of channels
parser.add_argument('--dec_in', type=int, default=98, help='decoder input size')
parser.add_argument('--c_out', type=int, default=98, help='output size')
